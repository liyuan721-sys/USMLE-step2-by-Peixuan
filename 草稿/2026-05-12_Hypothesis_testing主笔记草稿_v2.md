...
updated: 2026-05-12
type: 专题笔记草稿
status: 雏形 v2 / Null hypothesis + CI + OR 表判读章节已填充 / 待补 Type I-II / Power / p / 统计检验
---

# USMLE Hypothesis Testing 主笔记（草稿 v2）
...
> - 🚧 第 3 章：Power（统计效能）
> - 🚧 第 4 章：p-value 解读
> - ✅ 第 5 章：Confidence Interval（CI） — **v2 新填充**
> - ✅ 第 6 章：OR 表 / Forest plot 判读 — **v2 新增**
> - ✅ 第 7 章：Confounding（实战）— **v2 新增**
> - 🚧 第 8 章：统计检验选择（t / χ² / ANOVA / 回归）
> 
> **v2 素材来源**：Q3941（Null hypothesis）+ Q21248（OR/CI 判读 + Confounding）
...
## 5. Confidence Interval（置信区间）⭐ v2 新填充
...
**临床意义**：CI 0.91-16.84（很宽）说明真实 OR 可能从"无关联"到"17 倍"都有可能 → 数据不足以下结论。
...
> B. 咖啡可能与心律失常无关联
> C. 咖啡是保护因子
> D. 需要 cohort study 验证（CI 跨 1.0 仅意味着 statistical power 不够）
...
> 
> 表脚告诉你**哪些变量被当作 confounder 来调整**。这些被调整的变量本身在 multivariate 里可能"失去显著性"——这是设计使然，不是 bug。
> 
> **本题例子**：Q21248 表脚 "(adjusted for age, sex, **and CPB duration**)"
> - CPB duration 自己也在表里被列为"factor"
> - 但因为它本身就是 adjustment target → 调整后它的 OR 失去显著性是正常的
> - 这就是为什么 Bivariate CPB OR 11.84 → Multivariate OR 7.47 (CI 跨 1.0)
...
### 6.5 Confounding 在 OR 表里的诊断信号

> [!info] 怎么从表里看出"它是 confounder"
> 一个变量的关联在 **bivariate 很强**，但在 **multivariate 消失/大幅缩小且 CI 跨 1.0** → 说明它的"表观关联"主要来自**其他 confounder**（或它本身就是被调整的变量）。

### 6.6 Forest Plot（森林图）判读
...
### 7.4 Confounding 处理方法（设计 vs 分析阶段）
...
- 🔁 **相关错题**：
  ...
  - [[mistakes/uworld-mistakes#Q21248]] OR/CI 表判读 + Confounding（第 5, 6, 7 章主素材）
...
- 🌱 **TODO**：
  ...
  - 第 6 章 OR 表判读可能值得拆出独立笔记 [[完整笔记/专题笔记/_衍生_USMLE_OR_CI判读]]，等积累更多类似题再决定
...
| **8. Confounding 实战** ✅ | "调整了 A,B,C → 想 D,E,F 没调整呢？"= residual confounding |
| 9. 统计检验 | 🚧 待填 |
