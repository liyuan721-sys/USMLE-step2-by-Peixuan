...
> [!info] 文件起源
> 从 [[完整笔记/专题笔记/Ethics/Ethics_精简框架]] §十 拆出（2026-05-25 拆分，方案 2）。原文超大（4362 行），按主题分文件后单文件 ≈ 400 行可一次看完。
...
>
> 临床 informed consent 5 elements (见 [[完整笔记/专题笔记/Ethics/Ethics_精简框架]] §十一) + 研究专有 elements：
...
| **Cognitively impaired**（mild dementia / ID / severe psych）| Capacity 不足 / 易 manipulation | Surrogate consent + assent; **Mild ID 仍按 [[完整笔记/专题笔记/Ethics/Ethics_精简框架]] §二 facilitate understanding** 先评 patient 自己 capacity |
| **Pregnant women + fetuses** | 双 subject（母 + 胎）+ fetus 不能 consent | Minimal risk default + IRB 额外审 |
| **Minors** | 不能 legal consent + 长期 outcome 未知 | Parental/guardian consent + **age-appropriate assent**（详见 §4.1）|
...
### §4.1 Research with Minors — Age-Appropriate Assent SOP ⭐⭐（Q107485 锚定 — v1.2 新增）

> [!info] 反射钩
> Research enrolling minors (<18) → 反射 **双轨**：① **Informed consent from parents/guardians**（legal）② **Age-appropriate assent from minor**（agreement，不是 legal consent）。漏 assent = 钓饵。

#### Assent 定义

> **Assent** = minor 对参与 research 的 **agreement**（不是 legal consent — minor 无 legal authority to affirm/decline）。
> Like informed consent, **assent involves age-appropriate standardized procedure** 包括 expected risks / benefits / alternatives review。

#### Special Protections for Research with Minors — 3 大要素

| 要素 | 内容 |
|---|---|
| **Degree of risk to participants** | Minimal（completing a survey）vs substantial（taking experimental medication）— 风险越大保护越严 |
...
| **Procedures** | ① Informed consent from **parents/guardians** ② **Age-appropriate assent** from minor — 双轨同时 |
...
|---|---|---|---|
| **Who** | Parents / guardians | Minor 本人 | Parents |
| **Legal authority?** | ✅ Yes | ❌ No — 仅 agreement | ✅ Parents have authority |
| **Required?** | ✅ Always | ✅ Research required（除非 exception）| Treatment 不需要 minor agreement |
...
> **反例**（Q107485）：Observational study + no direct benefit to minor → **assent required**（即使风险低）。

#### "Research with minors" 4 大干扰项

| 干扰 | 钓饵原因 |
|---|---|
| Ensure resources to **treat medical needs** identified | ❌ 超出 study scope — research 不 obligate 提供 medical treatment beyond primary outcomes |
| **Cash / in-kind compensation** to parents | ❌ Compensation 常见但 **not required**；vulnerable populations 时 **excessive payment** = inducement |
| **No additional measures necessary** | ❌ 漏 assent — minors 必须 age-appropriate agreement |
| **Age-appropriate assent process** ⭐ | ✅ Research with minors 必备 |

#### CK 类比

买车 — 父母签合同（consent，legal authority）+ 子女说 "我同意"（assent，agreement）— 子女不能否决但应被尊重；治疗孩子打疫苗只需父母签字（不需要 assent）。
...
- 🔁 跨节联动：
  - 本文件 §一 COI ↔ [[完整笔记/专题笔记/Ethics/Ethics_精简框架]] §十一 Medical Legal（informed consent SOP）
  - 本文件 §三 Vulnerable populations ↔ [[完整笔记/专题笔记/Ethics/Ethics_精简框架]] §二 Capacity（cognitively impaired 评估）
- 🏥 跨学科：
