---
tags:
  - USMLE-Step2
  - Ethics
  - 老年医学
  - Discharge-Planning
created: 2026-05-27
updated: 2026-05-27
type: 专题笔记
---

# Ethics_Discharge_Planning 速查（6 档 disposition 对照 + 决策树）

> [!warning] 驱动错题
> - **5-25 Q20431** Home Health Care 首错（选 admit hospital — 升级 hospital 误用）
> - **5-27 Q9-2026-05-27** 同主题再错（选 SNF — 升级 institution 误用）→ 升 🔴 + 加 #考前必看
>
> **反复错根因**：disposition 决策树 + "medically stable" 概念 + "保 independence" 原则反射完全未建立 → 反射"elderly + fall + 担忧 → institution 升级"

---

## §0 目录

```
§一 6 档 Disposition 对照表 ⭐⭐⭐
§二 Disposition 决策树
§三 Home Health Care 三联触发词
§四 6 档触发条件 + 反 pattern
§五 老年 Care 升级红线
§六 Memory Hook + 类比
§七 真题反射钩库
§八 关联
```

---

## §一 6 档 Disposition 对照表 ⭐⭐⭐

| 档 | 设置 | 触发条件 | 服务 | Benefits / 注意 |
|---|---|---|---|---|
| 1 | **Outpatient follow-up** | Stable + 能出门 + 无 skilled needs | PCP / specialty 复诊 | 最 light — 适常规慢病管理 |
| 2 | **Home Health Care** ⭐ | **Medically stable + Homebound + Skilled needs**（三联）| Nursing / PT / OT / SW / wound / med management | 保 independence + 减 readmission + < SNF 贵 |
| 3 | **Assisted living** | Medically stable + ADL 部分需助 + 无 24/7 medical supervision | 协助洗澡 / 穿衣 / 吃药 + 社交 + 餐 | 介于 home 与 SNF |
| 4 | **Skilled Nursing Facility (SNF)** | Medically stable + **Severe CI / 不能 ADL / 持续 monitoring** | 24/7 nursing + rehab + 长期 medical care | 失 independence + 高 cost + 适 post-stroke rehab / 严重 dementia |
| 5 | **Hospice** | Terminal illness（预期 ≤ 6 mo）+ 放弃 curative | Symptom-focused + palliative + 家 / inpatient hospice / nursing home | 不强制 DNR；可保留 palliative tx |
| 6 | **Hospital admission** | **不 medically stable** / acute illness / 需 IV / 监护 / 手术 | 急性 medical / surgical | medically stable 时 admit = **net harm** ⚠️ |

---

## §二 Disposition 决策树 ⭐

```
Elderly + chronic condition / falls / 复杂多药
    │
    ▼
Q1: Medically stable?
    │
    ├─ ❌ 不 stable / acute illness → ⑥ Hospitalize
    │
    └─ ✅ Stable
           │
           ▼
       Q2: Terminal illness 预期 ≤ 6 mo + 放弃 curative?
           │
           ├─ ✅ 是 → ⑤ Hospice
           │
           └─ ❌ 否
                  │
                  ▼
              Q3: 需要 24/7 medical supervision?
              (severe CI / 不能 ADL / post-stroke rehab / 严重 dementia)
                  │
                  ├─ ✅ 是 → ④ SNF
                  │
                  └─ ❌ 否（mild CI / 能 ADL / minor injury）
                         │
                         ▼
                     Q4: ADL 部分需助但无 medical needs?
                         │
                         ├─ ✅ 是 → ③ Assisted living
                         │
                         └─ ❌ 否（仍能独立 ADL）
                                │
                                ▼
                            Q5: Homebound + 需 skilled care?
                            (PT / nurse / wound / med management)
                                │
                                ├─ ✅ → ② Home health care ⭐
                                │
                                └─ ❌ stable + 能出门 → ① Outpatient
```

---

## §三 Home Health Care 三联触发词 ⭐⭐⭐（CK 高频）

| 触发条件 | 含义 | Stem 暗号词 |
|---|---|---|
| **① Medically stable** | 不需 acute hospital level care | "medically stable" / "after recent discharge" / "vital signs normal" |
| **② Homebound + 需 intermittent monitoring** | 出门困难 / 限制 + 需定期专业上门 | "limited mobility" / "post-fall" / "递增 functional decline" / "递减社交" |
| **③ Skilled needs** | Nursing / PT / OT / SW / wound / med management 之一 | "多 medication" / "insulin" / "新 antidepressant" / "wound healing" / "post-stroke PT" |

**三联都满足 → home health care 首选**

### 3.1 Home Health Care 服务范围

| 服务 | 内容 |
|---|---|
| **Skilled nursing** | Med management / wound care / IV / 血糖监测 / 教育 |
| **PT (Physical Therapy)** | Fall prevention + gait training + 力量训练 |
| **OT (Occupational Therapy)** | ADL retraining + 辅助器具评估 + home safety assessment |
| **SW (Social Worker)** | Resources / 财务 / community connections / 心理支持 |
| **Speech therapy** | Post-stroke / dysphagia |

### 3.2 Home Health Care Benefits ⭐

| Benefit | 机制 |
|---|---|
| **Mental health + autonomy + independence 保持** | Aging in place — 不剥夺尊严 |
| **减 hospital readmission + LOS** | 早识别 symptoms + 防 functional decline |
| **减 healthcare spending** | < SNF (Medicare home health 比 SNF 便宜 50%+) |
| **减 nosocomial infection** | 避 hospital / nursing home environment |

---

## §四 6 档触发条件 + 反 pattern

### 4.1 ① Outpatient

**触发**：stable + 能出门 + 仅需常规复诊

**反 pattern**：
- 患者 medically stable + 多 chronic conditions + complex medication → **outpatient 不够 — 需 home health care**

### 4.2 ② Home Health Care ⭐

**触发**：见 §三 三联

**反 pattern**：
- "Pill organizer" / "family administer" — 不解决 inter-prescriber coordination + 不替代 skilled care
- "Office follow-up 4 weeks" — 高 risk 患者间隔太长

### 4.3 ③ Assisted Living

**触发**：medically stable + ADL 部分需助 + 无 medical supervision needs

**反 pattern**：
- Severe CI 不达 assisted living — 应 SNF
- Independent ADL 不需 assisted living — home + home health 即可

### 4.4 ④ SNF

**触发**：
- **Severe CI** + 不能独立 ADL
- **Post-stroke rehab**（需 daily PT + 24/7 nursing）
- **复杂 wound** 需 daily nursing
- **末期不能 home + 仍要 curative-leaning care**

**反 pattern** ⭐：
- ❌ "Mild CI + minor injury" 升级 SNF — 夺走 independence + 高 cost
- ❌ "Lives alone + 担心 fall" 升级 SNF — home health care 即可
- ❌ "Daughter 工作忙不能照顾" 升级 SNF — home health care 可 cover

### 4.5 ⑤ Hospice

**触发**：terminal illness 预期 ≤ 6 mo + 放弃 curative

**Hospice ≠ DNR**（不强制签 DNR）；**可保留 palliative tx**（symptom control / palliative radiotherapy）。

**反 pattern**：
- 同时 immunotherapy curative + hospice = ❌ 矛盾（hospice 必须放弃 curative）
- "All cancer treatment 停" 过严 — palliative tx 可继续
- Hospice + acute illness（MI / 感染）→ 可治不取消 hospice 资格

详见 [[完整笔记/专题笔记/Ethics/Ethics_EOL_Hospice与决策]]。

### 4.6 ⑥ Hospital Admission ⭐ 反 pattern 最常见

**触发**：**不 medically stable** / acute illness / 需 IV / 监护 / 手术

**反 pattern**：
- ❌ "Medically stable + admit for cognitive workup" = **net harm**
  - Cognitive assessment 可在家 / outpatient 完成
  - Admit medically stable 患者 = nosocomial infection + delirium + functional decline + iatrogenic + cost
- ❌ "Elderly + fall + 担忧" 反射 admit — medically stable 时应 home health care
- ❌ "多药 + 担心 ADE" 反射 admit — 应 home health med management + limit prescribers

---

## §五 老年 Care 升级红线 SOP ⭐

| 红线 | 应对 |
|---|---|
| **Medically stable** | 优先保 home + outpatient → 不 admit |
| **Mild CI + 能 ADL** | 优先保 independence → 不 SNF |
| **Acute illness / 不能 ADL / 持续 monitoring** | 才升级 SNF / hospital |
| **Terminal + 放弃 curative** | Hospice（不取代 acute care）|
| **Polypharmacy + ADE risk** | Limit prescribers（PCP 主管）+ home health med management（**不是** family administer / pill organizer）|

---

## §六 Memory Hook + 类比 ⭐

### 6.1 6 档强度类比

```
强度 0 ─ 完全独立 ─ ① Outpatient（自己开车去复诊）
       │
       ▼
强度 1 ─ 上门 skilled care ─ ② Home Health Care ⭐（专业护工每周来家）
       │
       ▼
强度 2 ─ 协助 ADL ─ ③ Assisted Living（搬进 ADL 协助型公寓）
       │
       ▼
强度 3 ─ 24/7 medical ─ ④ SNF（搬进养老康复院 24/7 监护）
       │
       ▼
强度 4 ─ Acute care ─ ⑥ Hospital（住院）
       │
       ▼
End-of-life ─ ⑤ Hospice（任何场所 — 家 / SNF / inpatient hospice — 关键是 goal-of-care 转向 symptom）
```

### 6.2 一句话锁 ⭐⭐⭐

- **"Medically stable + homebound + skilled needs → Home health care 首选"**（保 independence + 减 readmission + 省 cost）
- **"SNF 触发 = severe CI + 不能 ADL + 持续 monitoring；mild CI + minor injury ≠ SNF"**
- **"Medically stable + admit = net harm"**（nosocomial / delirium / functional decline）
- **"Hospice ≠ DNR；可保留 palliative tx"**

### 6.3 反 anchor 训练

不要反射"elderly + 担忧 → institution 升级"：
- 第一步永远是 **stable?** — stable 才有"保 home"的窗口
- mild CI ≠ SNF 触发；mild CI + medically stable + home health 是合理 spectrum

---

## §七 真题反射钩库

### 7.1 Q20431 / Q9-2026-05-27 — Home Health Care 双反复错卡

**Stem 模式**：elderly + medically stable + mild CI + 多 meds + fall + lives alone → 最佳干预？

**反 pattern 排除**：
- A admit → ❌ medically stable + admit = net harm
- B SNF → ❌ mild CI 不达 SNF 触发
- D family administer → ❌ daughter 工作忙 + 不解决 inter-prescriber
- E office 4 weeks → ❌ 高 risk 患者间隔太长

**正解**：**Home health care**（保 independence + skilled care + 减 readmission）

### 7.2 Q21141 — Hospice eligibility

**Stem 模式**：晚期 cancer + immunotherapy curative + palliative radiotherapy → 能 hospice 吗？

**核心**：Hospice 必须**停 curative**（immunotherapy）+ **可继续 palliative**（radiotherapy）+ **不强制 DNR**

### 7.3 Q20229 — Hospital Readmission Reduction

**Stem 模式**：CHF + 高 Medicare readmission penalty → 减 30-day readmission 策略？

**正解**：Telephone-based outreach by care coordinator within 2 weeks
- 同 patient-side intervention 哲学（vs provider-side communication）
- Discharge summary to PCP 不解决 patient 端 loss to follow-up

### 7.4 Q107185 — Geriatric Polypharmacy

**Stem 模式**：83yo + 多 specialists + 10+ meds → reduce ADE risk

**正解**：**Limit number of prescribers**（PCP 主管 + 与 specialists 协调）
- Pill organizer / family administer ❌ 不解决 inter-prescriber coordination

### 7.5 Q19909 — Transfer of Care + Loss to Follow-up（lung nodule incidental finding 后未 attend）⭐⭐⭐（v1.1 增量）

**Stem 模式**：73F CAP medically stable + 出院时 incidental lung nodule + outpatient CT 未 attend → biopsy lung adenocarcinoma → 减 future similar errors

**正解**：**A. Explaining importance of primary care follow-up after discharge**
- < 20% inpatients with incidentally detected lung nodules complete postdischarge follow-up
- Loss to follow-up = common cause of preventable adverse outcomes / delayed dx

**反 pattern 排除**：
- B family members → ❌ Independent cognitively intact patient → 直接 engage（不绕过 patient）
- **C monitor pending results for all discharged patients** → ❌ ⭐ **超 inpatient scope** + 增 provider workload + PCP 已通过 EMR 收 abnormal outpatient test log
- D postpone discharge → ❌ Medically stable + postpone = net harm（nosocomial / functional decline / resource waste）

---

## §七.x Transfer of Care + Loss to Follow-up SOP ⭐⭐⭐（v1.1 增量 — Q19909 锚定）

> [!success] Transfer of Care 是 discharge 最高 risk 时点 — loss to follow-up 是 preventable adverse outcomes 主因

### 7.x.1 Transfer of Care SOP 5 步

| 步骤 | 内容 |
|---|---|
| **① Timely primary care follow-up** | **< 4 weeks** post-discharge — 完成 pending studies + lab tests + new medications + diagnoses |
| **② High-quality patient counseling**（discharge process）| Face-to-face discourse + 非言语 cues + patient-centered + interactive verification |
| **③ Teach-back** ⭐ | Patient repeat back understanding — 防 omission（同 Q19788 medication counseling）|
| **④ Discharge summaries to PCP** | Routed via EMR — PCP 在 patient posthospital visit 时 review abnormal results |
| **⑤ Address barriers** | 交通 / 经济 / 认知 / 语言 → 改善 attendance |

### 7.x.2 High-Quality Counseling 4 大特征

| 特征 | 详解 |
|---|---|
| **Face-to-face discourse** | Allow responses to nonverbal cues |
| **Patient-centered interaction** ⭐ | Tailored to patient's current motivations, goals, ability |
| **Interactive verification** ⭐ | Inviting questions + **teach-back** that summarizes understanding |
| **Empathetic + culturally appropriate** | 减 barriers |

### 7.x.3 反 pattern 4 类（CK 高频钓饵）

| 反 pattern | 例 | 为什么错 |
|---|---|---|
| **① Provider-side monitoring for ALL patients** | Q19909 Choice C | **超 inpatient scope**（inpatient = acute care，不是 chronic monitoring）+ 增 provider workload + PCP 已通过 EMR 系统监控 |
| **② Inform family** when independent + cognitively intact | Q19909 Choice B | 仅适 cognitive impairment 或 patient consent；independent 患者直接 engage |
| **③ Postpone discharge** medically stable | Q19909 Choice D | Medically stable + postpone = **net harm**（nosocomial / functional decline / resource waste） |
| **④ Rely on radiology to call patient**（passive system） | — | Loss to follow-up rate 高；患者**主动 understand + commit** 是关键 |

---

## §九 Patient-side vs Provider-side Intervention 哲学 ⭐⭐⭐⭐（v1.1 增量 — multi-anchor 元方法）

> [!danger] USMLE 永远选 **Patient-side intervention** primary，provider-side 是 adjunct ⚠️
> 5-25 Q20229 + 5-27 Q19909 两次反复错都是**反射选 provider-side**（discharge summary / monitor all pending results）— **patient-side 哲学反射尚未建立**

### 9.1 反 pattern 总览 — Provider-side Reflexive Errors

| 错题 | Provider-side 钓饵（错）| Patient-side 正解 |
|---|---|---|
| **Q20229** (5-25) CHF readmission | D "Send discharge summary to PCP" | D "**Telephone outreach by care coordinator within 2 weeks**" |
| **Q19909** (5-27) Lung nodule loss to follow-up | C "Monitor pending results for all" | A "**Explain importance of primary care follow-up**"（patient counseling）|
| **Q22061** (5-25) Patient demand unnecessary MRI | E "Diagnose + offer Rx muscle relaxants" | A "**Test nerves with commentary**"（address patient's specific concern）|
| **Q19788** (5-27) Sound-alike drug error counseling | B "Avoid combinations" / D "Family explanation" | C "**Teach-back**"（patient verifies understanding）|

### 9.2 共同识别钩 ⭐

| Stem 信号 | 反射 |
|---|---|
| "Loss to follow-up / readmission / nonadherence" | **Patient-side counseling**（不是 provider-side communication / monitoring）|
| "Patient demand X / asks specific worry" | **Address patient's specific concern with commentary**（不是替代 dx）|
| "Medication error potential / sound-alike" | **Teach-back**（不是 family explanation / printed material）|
| "Confused / forgot / misunderstood" | **Patient-centered counseling + teach-back**（不是 single-direction explanation）|

### 9.3 为什么 Patient-side > Provider-side ⭐

| 维度 | Patient-side | Provider-side |
|---|---|---|
| **直接 address 行为驱动？** | ✓ 是 — patient 是 outcome 的直接执行者 | ❌ 间接 — 信息传到 provider 仍要 patient 行动 |
| **Scalable？** | ✓ 每次 visit / discharge counseling | ❌ Monitor ALL patients 超出 inpatient scope |
| **Resource burden？** | 低（counseling 是 standard practice）| 高（systematic monitoring 增 workload）|
| **Evidence base？** | 强（teach-back + telephone outreach reduce readmission 显著）| 较弱（discharge summary 不解决 patient 端 loss）|

### 9.4 Patient-side Intervention 工具家族 ⭐

| 工具 | 何时用 | 锚题 |
|---|---|---|
| **Patient-centered counseling**（face-to-face + nonverbal）| Discharge + complex chronic | Q19909 |
| **Teach-back**（patient verifies understanding）| Sound-alike drug + complex regimen | Q19788 |
| **Telephone outreach within 2 weeks**（care coordinator）| High readmission risk + chronic disease | Q20229 |
| **Address specific concern + ongoing commentary** | Patient demand unnecessary intervention | Q22061 |
| **Motivational interviewing + SMART goal** | Lifestyle modification | Q22080 / Q12-2026-05-26 |
| **Identify perceptions about causes**（first step PCC）| Cross-cultural care | Q21242 |

### 9.5 一句话锁 ⭐⭐⭐

- **"Loss to follow-up / readmission → Patient-side counseling（不是 provider-side monitoring）"** ⭐
- **"Inpatient scope = acute care；chronic outpatient monitoring 不是 inpatient 责任"** ⭐
- **"Teach-back + telephone outreach + patient-centered counseling = patient-side 3 大武器"** ⭐
- **"Provider-side communication 是 adjunct，不是 primary"** ⭐
- **类比：药店配药后给患者** — counseling 让患者**理解 + 承诺按时吃**（patient-side）> 药店每天打电话提醒所有客户（provider-side monitoring 超 scope）

---

## §八 关联

- 🔁 **同主题错题**：
  - [[mistakes/uworld-mistakes#^Q20431]] Home Health Care 5-25 首错 🔴
  - [[mistakes/uworld-mistakes#^Q9-2026-05-27]] 5-27 SNF 反复错升 🔴 + #考前必看
  - [[mistakes/uworld-mistakes#^Q21141]] Hospice eligibility（5-25）
  - [[mistakes/uworld-mistakes#^Q20229]] Telephone outreach readmission（5-25）
  - [[mistakes/uworld-mistakes#^Q107185]] Geriatric polypharmacy limit prescribers（5-25）
  - [[mistakes/uworld-mistakes#^Q107149]] Inpatient fall prevention (5-26 — 不同 setting inpatient vs discharge home)
- 📚 **主笔记**：
  - [[完整笔记/Peixuan分科笔记/Ethics_Master]]
  - [[完整笔记/专题笔记/Ethics/Ethics_EOL_Hospice与决策]] §2.9 home health care 哲学
  - [[完整笔记/专题笔记/Ethics/Ethics_Communication_Counseling]] §12.6 Patient-Centered Care
- 🏥 **跨学科**：
  - [[完整笔记/Peixuan分科笔记/内分泌]]（DM polypharmacy + insulin med management）
  - [[完整笔记/Peixuan分科笔记/心内]]（CHF readmission high-risk + diuretic monitoring）
  - 老年病学 / Geriatric Medicine
- 🌱 **TODO**：
  - 累积 ≥ 5 道 disposition 题（Hospital / SNF / Home health / Hospice / Assisted living）→ 在 §七 真题反射钩库 加完整型录
  - 与 [[完整笔记/专题笔记/USMLE/USMLE_老年Fall3层防护]] 联动（fall prevention 在 inpatient / SNF / home 三档不同策略）

---

## 版本记录

| 版本 | 日期 | 关键变更 |
|---|---|---|
| **v1（首建）** | 2026-05-27 | 6 档 disposition 对照 + 决策树 + Home Health Care 三联触发词 + 老年 Care 升级红线 + Memory Hook + 真题反射钩库；驱动错题 Q20431 + Q9-2026-05-27 反复错升 🔴 |
