...
> | 系统 | 反应 | 特点 |
> |---|---|---|
> | **Titratable acid**（磷酸盐）| HPO4²⁻ + H⁺ → H2PO4⁻ | **配额固定**（受滤过磷酸盐限），代酸时几乎不能多排 |
> | **NH3 / NH4⁺（铵）** ⭐ | 近端用谷氨酰胺合成 NH3；NH3 + H⁺ → NH4⁺ | **可调主力**：代酸时上调氨生成（ammoniagenesis），容量大、可数倍↑ → 肾"加大排酸"的真正阀门 |
>
> **核心：排一个酸 = 再生一个新 HCO3（同一反应两面）⭐**
> 近端小管代谢 **1 分子谷氨酰胺 → 2 NH4⁺（→尿，带走 H⁺）+ 2 新 HCO3⁻（→血，补碱）**。所以"多排 NH4⁺"和"多还 HCO3 回血"是绑定的。
>
> **区分两件事（别混）：**
> - **HCO3 重吸收**（reabsorption）= 把滤过的 HCO3 捡回来（近端 + 碳酸酐酶 CA，代酸时 CA↑）
> - **HCO3 再生**（generation）= 靠 NH4⁺ / titratable acid 排酸**净造出新 HCO3**（远端）
>
> **电中性配对：HCO3 重吸收↑ ↔ 尿 Cl⁻ 排泄↑**（不是重吸收！）—— Q13842 干扰项"尿 Cl 重吸收↑"方向写反。
>
> **时间轴**：呼吸代偿分钟级（Kussmaul → PaCO2↓）；**肾代偿数小时启动、数天达峰**。所以 AAA 夹闭后数小时的乳酸酸中毒，肾的反应 = **正在上调 NH4⁺ 生成**（这就是 Q13842 选"↑尿 NH4⁺"的原因；真把 HCO3 补回正常要几天）。
>
> 一句话：**"肾排酸不排游离 H⁺，靠 NH4⁺（可调主力）+ 磷酸盐（固定）；每排 1 个 NH4⁺ = 再生 1 个 HCO3 回血；HCO3 重吸收↑必然尿 Cl 排泄↑；呼吸代偿分钟级、肾代偿天级。"**
