---
tags:
  - USMLE-Step2
  - Psych
  - 神经
created: 2026-05-22
type: 专题笔记
---

# 抗精神病药四大 EPS 诊断治疗对照

> [!info] 一句话总览
> 抗精神病药（含 D2 阻断的止吐药）→ 锥体外系反应 (EPS)。**按起病时间轴四分**：dystonia → akathisia → parkinsonism → TD。**每类治疗药不同，混用会加重**（最高频死法 = 抗胆碱能用在 TD 上）。读题先扫「用了什么药 + 多久」+「动作类型」，时间窗 + 动作类型双锁定。

---

## 一、核心机制 — DA / ACh 跷跷板

```
   抗精神病药 / 止吐药
        ↓ 阻断 D2 (穿 BBB)
   纹状体 / 基底节  DA↓ ⇄ ACh 相对↑
        ↓
   不自主运动 / 张力异常 (EPS)
        ↓
   按时间窗演化：
   小时-天   → Acute dystonia
   天-周     → Akathisia
   周-月     → Drug-induced parkinsonism
   月-年     → Tardive dyskinesia (TD)
```

> [!tip] 治疗逻辑（机制层 vs 临床决策优先级）
> 机制层：
> - **Acute dystonia** / **DIP** = DA↓·ACh↑ → 压 ACh 或补 DA 都能改善症状
> - **Akathisia** = β-能驱动 → β 阻断（**propranolol**）
> - **TD** = **慢性 D2 阻断 → D2 受体超敏（supersensitivity）**，与急性 DA↓·ACh↑ **机制不同** → 抗胆碱**无效甚至加重** → 用 **VMAT2 抑制剂**减少突触前 DA 释放，或**减量 / 换低 D2 亲和药**
>
> **临床决策优先级**（⭐ USMLE 真考点 — 与机制层 ≠ 同义）：
> - **Acute dystonia**：抗胆碱 IM **一线**（机制对症 = 临床一线，急救场景）
> - **DIP**：⚠️ **减/停元凶 + 换低 D2 SGA 一线**（不是抗胆碱）— 元凶在体外，撤药根治；抗胆碱只是症状性辅助，老年慎用
> - **Akathisia**：propranolol **一线**（非抗胆碱）
> - **TD**：VMAT2 + 换药 **一线**（抗胆碱反加重）
>
> **抗胆碱专属一线阵地 = Acute dystonia only**。看到其他三个 EPS 选抗胆碱 = 钓饵。

---

## 二、四大 EPS 速查金表（背熟它）

> [!danger] ⭐ 优先级铁律（防"benztropine 万能"反射）
> **抗胆碱（benztropine / diphenhydramine）= 一线 ONLY 对 Acute dystonia**。其他三个 EPS：
> - **Akathisia → propranolol 一线**（benztropine 不是常规备选 — 作用弱、副作用多）
> - **DIP → 减/停元凶 + 换低 D2 SGA 一线**（benztropine **二线辅助**，老年慎用）
> - **TD → ❌ benztropine 反加重**（D2 超敏机制 ≠ ACh 相对↑）
>
> **下表"一线治疗"列严格按 first-line 写**；二线 / 备选放在括号内 + 标注角色。

| EPS | 起病时间 | 经典表现 | **一线治疗** ⭐ | Stem buzzwords |
|---|---|---|---|---|
| **Acute dystonia** 急性肌张力障碍 | **小时–天** | 头颈持续收缩、torticollis 斜颈、**oculogyric crisis 动眼危象**、舌肌痉挛、jaw clenching；神志清醒 | **Benztropine** 或 **diphenhydramine**（IM 数分钟起效）| "stiff neck"、"eyes rolled up"、"tongue protruding"、术后/止吐药打完 1 小时 |
| **Akathisia** 静坐不能 | **天–周** | **主观**内在不安、坐立不能、踱步、fidget；加量更糟 | **① 减量元凶 + ② propranolol**（β 阻断）⭐；难治二线 mirtazapine / 苯二氮卓 / benztropine（**弱效，非常规备选**） | "can't sit still"、"pacing"、"restless"、加量后更糟 |
| **Drug-induced parkinsonism** 药源性帕金森 | **周–月** | 静止性震颤、**铅管样**僵直、运动迟缓、慌张步态、面具脸 | **① 减/停元凶 + ② 换低 D2 SGA**（quetiapine / clozapine）⭐；不可减药 / 残余症状二线辅助：**amantadine**（首选辅助）/ **benztropine**（老年慎用 — 谵妄/尿潴留风险） | "tremor at rest"、"rigidity"、"mask-like face"、"shuffling gait"、长期抗精神病药 |
| **Tardive dyskinesia (TD)** 迟发性运动障碍 | **月–年（>6 月）** | **口面部**舞蹈样：lip smacking、吐舌、grimacing、咀嚼样；肢体/躯干 choreoathetoid 运动；神志清醒 | **① 结构性：减/停元凶 + 换低 D2 SGA（clozapine ⭐ 优先 / quetiapine 次选）⭐** + **② 药物性：VMAT2 抑制剂（valbenazine / deutetrabenazine FDA 批准）⭐** — **两层并行**（不是三选一）；⚠️ 突然减药 → withdrawal-emergent dyskinesia 短期加重 → 减药要慢 + 同时上 VMAT2 缓冲；★抗胆碱反加重 / 加量原药短期掩盖长期恶化★ | "long-term use"、"lip smacking"、"tongue protrusion"、"chewing"、>6 月用药 |

> [!warning] 时间窗 + 治疗药速记（精修版）
> **"时-天 dystonia → 抗胆碱 ⭐；天-周 akathisia → propranolol（不是抗胆碱）；周-月 DIP → 减元凶 + 换低 D2（抗胆碱二线）；月-年 TD → VMAT2 + 换药（抗胆碱反加重）"**
>
> **抗胆碱专属阵地** = **Acute dystonia 一线 only**。其他三个看到选项有 benztropine → 不是首选。

> [!danger] 用户 Q9 错题元教训（2026-05-24）
> 用户 5-24 浩斯宾 Q9（DIP 处理）误选 benztropine — 反射来源 = 旧版本金表"减量 / amantadine / benztropine"三项**并列**让人误以为 benztropine 是 DIP 一线之一。**新版金表已收紧：DIP 一线 = 减元凶 + 换低 D2 SGA**，benztropine 是二线辅助且老年慎用。教训：**速查表的简化并列不等于优先级相等** — 始终核对 §三 药物角色表 + §4.3 阶梯。

---

## 三、治疗药物 → 治什么 / 不治什么 / 加重什么

| 药 | 机制 | 治 | 加重 / 无效 |
|---|---|---|---|
| **Benztropine** 苯托品 | 抗胆碱 | **Acute dystonia ⭐ 一线**（IM 数分钟起效）；**DIP 二线辅助**（仅在不可减元凶/换低 D2 时考虑，**老年慎用** — 谵妄/尿潴留/视模糊/加重痴呆）；idiopathic PD 震颤为主（老年慎用） | ❌ **TD（反加重）**；❌ Akathisia（弱效非常规备选，propranolol 优先） |
| **Diphenhydramine** 苯海拉明 | 抗组胺 + 强抗胆碱 | **Acute dystonia 一线替代**（同 benztropine，镇静更强可助焦虑） | ❌ TD；❌ 慢性 EPS（镇静累积） |
| **Amantadine** 金刚烷胺 | 促 DA 释放 + 弱 NMDA 拮抗 | **DIP 二线辅助首选**（优于 benztropine — 无抗胆碱副作用，老年友好）；levodopa 诱导 dyskinesia；治流感（M2 通道，现少用） | ⚠️ PD psychosis 中**加重**（多巴胺能药） |
| **Propranolol** 普萘洛尔 | β 阻滞 | **Akathisia ⭐ 一线** | — |
| **Mirtazapine** | 5-HT2A 拮抗 + α2 拮抗 | **Akathisia 二线**（证据较 propranolol 弱 — 当 propranolol 禁忌如哮喘 / 严重 bradycardia 时） | — |
| **Benzodiazepine** | GABA-A 激动 | Akathisia 三线备选（短期，避免依赖） | — |
| **Valbenazine / Deutetrabenazine** | **VMAT2 抑制**（突触前↓ DA 释放）| **TD 一线**（FDA 批准）| — |
| **Clozapine** | 极弱 D2 + 多受体（5-HT2A / M1 / H1 / α1） | **TD 换药首选 ⭐**（证据最强 + 自己几乎不致 TD + 兼治难治性精分；需 ANC 监测）；DIP 换药；PD psychosis 第 5 步备选 | ⚠️ Agranulocytosis（ANC 监测）/ 癫痫（剂量依赖）/ 心肌炎 / 流涎 |
| **Quetiapine** | 极弱 D2 SGA | **DIP / TD 换药一线**（减/停原元凶后换）；**PD psychosis 第 5 步首选**（availability + safety 最佳） | 镇静 / 体重↑ / 直立性低血压 |
| **Haloperidol / Risperidone / Olanzapine** | 中-高 D2 拮抗 | ❌ **EPS 治疗题里永远是错答案**（自己就是元凶）；PD psychosis 禁用 | 加重全部 EPS |
| **Levodopa-carbidopa** | DA 前体 | **Idiopathic PD 一线**；❌ **DIP 中禁用**（D2 被阻断突破不了 + 加重 psychosis） | DIP / PD psychosis |

> [!danger] ★最高频死法★
> **"用对药、认错病"** —— Benztropine / diphenhydramine 治 dystonia / parkinsonism 一流，**用到 TD 上无效甚至加重**。看到「lip smacking / 吐舌 / 长期用药数月-数年」→ TD → 选 VMAT2 / 换药，**绝不选 benztropine**。

---

## 四、各亚型快卡

### 4.1 Acute Dystonia（急性肌张力障碍）

- **谁致病**：D2 拮抗药（高效价 FGA 如 haloperidol；止吐药如 prochlorperazine / metoclopramide）
- **何时**：用药后**小时-天**
- **动作**：**头颈为主**的持续性肌肉收缩 —— torticollis、oculogyric crisis（眼上翻定住）、舌外伸、jaw clenching
- **关键**：神志清醒、生命体征正常
- **一线**：**Benztropine IV/IM** 或 **diphenhydramine IV/IM**（数分钟起效）；可与 D2 止吐药同给作预防
- **陷阱**：误当低钙抽搐（"muscle spasm" 通用词诱导）→ 答案藏在**用药史**

### 4.2 Akathisia（静坐不能）

- **谁致病**：任何抗精神病药（剂量依赖，**最常见 EPS**）；SSRI 也可诱发
- **何时**：换药 / 加量后**天-周**
- **动作**：**主观**内在不安 + 客观 fidget / pacing；不像 dystonia 是被动收缩
- **关键陷阱**：**加量更糟**（误当激越加抗精神病药 = 火上浇油）；易被误读为"焦虑 / 激越"
- **一线**：**减量 + propranolol**；备选 benztropine / 苯二氮卓
- **方向性原则**：剂量是"减"不是"加"；换药换"更低 EPS 风险"不是更高（绝不换 haloperidol）

### 4.3 Drug-Induced Parkinsonism（药源性帕金森）

- **谁致病**：高效价 FGA 最高（haloperidol / fluphenazine）；任何 D2 阻断药（risperidone / metoclopramide 等）
- **何时**：用药 / 加量后**周-月**
- **动作**：**静止性震颤 + 铅管样僵直 + 运动迟缓 + 慌张步态 + 面具脸 + 拖步** —— 与特发性帕金森 (IPD) 形态相似
- **vs Catatonia 的 negativism**：parkinsonism 僵是**被动恒定的铅管 / 齿轮样**阻力；catatonia negativism 是**主动对抗、越推越顶（resistance ∝ force）**
- **DIP 治疗阶梯**：① **减量**（首选，常不可行）→ ② **换低 D2 SGA**（quetiapine / clozapine）→ ③ **amantadine** 或 **benztropine** 对症

> [!danger] ★Levodopa 在抗精神病药致 DIP 中禁用★
> ① 增加 DA → **对抗抗精神病作用、加重精神病**；② DIP 根因是 D2 受体**阻断**（不是 DA 缺失），加 DA 突破不了阻断。**Levodopa 完全不在 DIP 治疗阶梯里**。详见 [[mistakes/uworld-mistakes_2026-05#^Park1-2026-05-22]]。

**IPD vs DIP 三轴鉴别（「称·程·药」口诀）**

| 维度 | **Idiopathic PD** | **Drug-Induced Parkinsonism** |
|---|---|---|
| **对称性** | **不对称**（一侧主导）⭐ | **对称** |
| **病程** | 数月-数年缓慢进展 | 数周-数月（与用药 / 加量时序相关）|
| **致病药** | 无 | 抗精神病药（FGA > SGA）/ metoclopramide / 罕见 valproate·lithium |
| 嗅觉减退 hyposmia | 早期常见、有特异性 | 一般无 |
| 治疗 | **Levodopa-carbidopa 一线** | **减量 / 换低 D2 SGA**；**禁** levodopa |
| 可逆性 | 不可逆、进展性 | **撤药可逆**（数周-数月）|

完整对照见 [[mistakes/uworld-mistakes_2026-05#^Park2-2026-05-22]]。

**TRAP 四联**（IPD 临床诊断）：**T**remor at rest / **R**igidity / **A**kinesia-bradykinesia / **P**ostural instability

### 4.4 Tardive Dyskinesia（迟发性运动障碍）

- **谁致病**：长期 D2 阻断 → D2 受体**超敏（supersensitivity）**；FGA > SGA；**risperidone 在 SGA 中较高**；**clozapine / quetiapine 低**；metoclopramide 长期用也可致 TD
- **何时**：用药**月-年（>6 月）**；"tardive" = 迟到
- **动作**：**口面部**为典型 —— lip smacking、吐舌、grimacing、咀嚼样；肢体/躯干 choreoathetoid 运动
- **治疗结构（与 DIP 平行 — 两层并行不是三选一）**：

  **① 结构性（治本 — 减少 D2 阻断刺激）**：
  - **减/停元凶**（多数不能纯停 — 精神病复发风险）
  - **换低 D2 SGA**：**Clozapine ⭐ 优先**（TD 证据最强 + 自己几乎不致 TD + 兼治难治性精分；需 ANC 监测）；Quetiapine 次选
  - ⚠️ **Withdrawal-emergent dyskinesia 陷阱**：突然减药 → 残留 D2 超敏暴露 → 短期 TD 加重 → **减药要慢 + 同时上 VMAT2 缓冲**

  **② 药物性（治标 — FDA 批准对症）**：
  - **Valbenazine**（一日一次）⭐
  - **Deutetrabenazine**（一日两次）⭐
  - Tetrabenazine（off-label，副作用较新 VMAT2 多 — 现少用）

  **避免（USMLE 钓饵）**：
  - ❌ **抗胆碱（benztropine）反加重**（D2 超敏机制 ≠ 急性 ACh 相对↑）
  - ❌ **加量原药短期掩盖、长期恶化**
  - ❌ **Levodopa 不在 TD 治疗里**

- **DIP vs TD 治疗对比（结构相同 + 药物层差异）**：

| 维度 | **DIP** | **TD** |
|---|---|---|
| 结构性一线 | 减/停元凶 + 换低 D2 SGA | 减/停元凶 + 换低 D2 SGA（**Clozapine 优先**）|
| 药物性辅助 | Amantadine（首选）/ benztropine（老年慎用）| **VMAT2 抑制剂** ⭐（valbenazine / deutetrabenazine） |
| 抗胆碱角色 | 二线辅助（老年慎用）| ❌ **反加重** |
| 减药特殊性 | 撤药数周-数月可逆 | ⚠️ 突然减药 → withdrawal-emergent 短期加重；减药要慢 |
| 换药首选 | Quetiapine ≈ Clozapine（均低 D2 即可）| **Clozapine 优先**（证据最强 + 几乎不致 TD）|

- **★死穴★**：**抗胆碱能无效甚至加重 TD**；加量抗精神病药短期"掩盖"长期恶化 → 都是错答案
- **包装陷阱**：出题人常用"情绪 / 行为变化 / 同事觉得她生气 / 自卑 / 独自吃饭"等心理叙事包装客观的不自主运动 → 看到长期用药 + 口面动作直接锁 TD，别被情绪线带跑

---

## 五、抗精神病药 TD 风险分层

| 类别 | 代表 | TD 风险 |
|---|---|---|
| FGA（高效价）| haloperidol、fluphenazine | **最高** |
| FGA（低效价）| chlorpromazine | 高（另有胆汁淤积性黄疸、QT、抗胆碱副作用）|
| SGA（较高）| **risperidone**、paliperidone | 中-高 |
| SGA（中等）| olanzapine、aripiprazole | 中 |
| SGA（低）| **clozapine、quetiapine** | **最低** |
| 止吐药（D2 拮抗）| metoclopramide、prochlorperazine | 长期用可致 TD |

> [!tip] 选药口诀
> **"已得 TD 想换药 → quetiapine 或 clozapine；多药失败 + 难治精分叠加 TD → clozapine 一药两担。"**

> [!info] 完整 SGA 三维副作用（代谢 / EPS / QT）UW 官方表
> 本节只展示 TD 风险纵切；**全 SGA 三维副作用对比表**（含代谢综合征、EPS、QT 延长 7 药对照）见 [[完整笔记/专题笔记/psych/psych_精神科药物副作用速查#2.2 SGA 各药副作用表]]（图 UW 来源 → [[mistakes/uworld-mistakes_2026-05#^Park1-2026-05-22]]）。该表与本节 TD 风险分层对齐：**Risperidone EPS 高** ↔ TD 风险中-高；**Clozapine / Quetiapine EPS 极低** ↔ TD 风险最低。

---

## 六、读题 SOP（防止误判）

```
看到「不自主运动 / 张力异常」病人：
  ① 先扫用药史：用过什么 D2 阻断药？多久了？
  ② 看动作类型：头颈持续？踱步？震颤+僵？口面舞蹈？
  ③ 时间窗 + 动作 双锁定 → 选治疗药
  ④ 反包装：情绪 / 心理叙事可能是不自主运动的伪装
```

- ❌ 别条件反射"加量抗精神病药" —— akathisia 会更糟、TD 会长期恶化
- ❌ 别给 TD 抗胆碱能 —— 无效 / 加重
- ❌ 别给 EPS 治疗题选 haloperidol —— 自己是元凶
- ❌ 别误当低钙抽搐 / 焦虑 / 心理问题 —— 钥匙都在用药史

---

## 七、易混陷阱（普适）

| 陷阱 | 真相 |
|---|---|
| 抗胆碱能（benztropine）治所有 EPS | ❌ **一线 ONLY 对 acute dystonia**；DIP 是二线辅助（先减元凶 + 换低 D2 SGA）；Akathisia 弱效非常规备选（propranolol 一线）；**TD 反加重** |
| Akathisia = 焦虑 / 激越 → 加量 | ❌ 加量更糟；**减量 + propranolol** |
| TD = 长期 EPS → 给 benztropine 试试 | ❌ TD 机制是 D2 超敏不是 ACh↑；抗胆碱无效 |
| Haloperidol 既然 D2 强、能"治" EPS | ❌ 永远是错答案，自己就是元凶 |
| Lithium 致震颤 = TD | ❌ Lithium tremor 是细震颤，**不是** TD 那种舞蹈样 |
| 加量"掩盖" TD = 处理对 | ❌ 短期掩盖、长期恶化，是陷阱选项 |
| 病人"心情不好、不愿出门" → 心理问题 | ❌ 可能是 TD 的"包装"；先看用药史 + 口面动作 |

---

## 八、与"高热 + 肌强直急症"的衔接

EPS 是"不发热 / 神志清"族；以下急症族 = "**发热 + 自主神经不稳 + 神志改变**"：

| 急症 | 致病 | 起病 | 关键 | 治疗 |
|---|---|---|---|---|
| **NMS** | D2 阻断（任何抗精神病药）| 数天 | 高热 + 铅管样僵直 + AMS + 自主神经不稳 + CK↑ + 白细胞↑ + 肌红蛋白尿 | 停药 + dantrolene / bromocriptine；难治 / 重症可 ECT |
| **5-HT 综合征** | 5-HT 药物（SSRI/SNRI/MAOI/林奈+其他）| 数小时 | 高热 + **下肢为主 clonus / 反射亢进** + AMS | 停药 + cyproheptadine |
| **Malignant catatonia** | catatonia 叠加恶性化 | 急 | 高热 + 严重肌强直 + 自主神经不稳 + 横纹肌溶解 | Lorazepam 试，危重 **ECT 急迫上**；与 NMS 高度重叠 |
| **Malignant hyperthermia** | 麻醉触发（succinylcholine / 卤代醚）| 麻醉中 | ETCO₂↑↑ + 高热 + 肌强直 | Dantrolene |

> [!info] 见 [[mistakes/uworld-mistakes_2026-05#^Q12203]] NMS 实验室表现（白细胞↑、肌红蛋白尿）；详细高热肌强直急症鉴别待生成 `_衍生_高热肌强直急症鉴别`。

---

## 🔗 关联

- 🔁 同主题错题：
  - [[mistakes/uworld-mistakes_2026-05#^Q2505]] Acute Dystonic Reaction（dystonia 急救：D2 止吐药惹的祸用 1 代抗组胺的抗胆碱副作用解）
  - [[mistakes/uworld-mistakes_2026-05#^Q11898]] Akathisia 静坐不能（akathisia 治疗端：减量 + propranolol；haloperidol 治 EPS 永远错）
  - [[mistakes/uworld-mistakes_2026-05#^Park1-2026-05-22]] DIP 处理（错 — 选 levodopa；正解换 quetiapine；**levodopa 在 DIP 中禁忌**）
  - [[mistakes/uworld-mistakes_2026-05#^Park2-2026-05-22]] IPD vs DIP 鉴别（做对 — 三轴「称·程·药」+ TRAP 四联）
  - [[mistakes/uworld-mistakes_2026-05#^Q9-2026-05-24]] DIP 处理 第 2 次（2026-05-24 浩斯宾错 — 选 benztropine；正解减/停 risperidone + 换 quetiapine/clozapine；**急性 EPS = 救症状抗胆碱 vs 慢性 EPS = 救通路减元凶**两端钓饵都踩过 — 与 Park1 选 levodopa 同主题第 2 次钓饵）
  - [[mistakes/uworld-mistakes_2026-05#^Q16066]] TD & EPS 四兄弟鉴别（5-20 错题 — TD 识别 + benztropine 加重 TD 陷阱）
  - [[mistakes/uworld-mistakes_2026-05#^Q2518]] TD 处理（5-22 做对 — 减量不可行 + VMAT2 无效 → 换 clozapine；多药反应差时 clozapine 优先）
  - [[mistakes/uworld-mistakes_2026-05#^Q12203]] NMS 实验室表现（EPS 谱系外、高热急症族；同 D2 阻断机制）
- 📚 主笔记：[[完整笔记/Peixuan分科笔记/psych]] / [[完整笔记/Peixuan分科笔记/neuro]]
- 🏥 跨学科：
  - [[完整笔记/Peixuan分科笔记/消化补充内容]]（metoclopramide / prochlorperazine 作止吐药 → EPS / TD；化疗止吐高频）
  - [[完整笔记/Peixuan分科笔记/外科]]（malignant hyperthermia 麻醉急症 — 同"高热 + 肌强直"族）
- 🌱 TODO（待积累 / 后续衍生）：
  - ✅ Parkinsonism 错题已补齐（Park1 / Park2 — 2026-05-22 入库 + §4.3 完整扩展）
  - 后续积累 Parkinson-plus 综合征（PSP / MSA / CBD / DLB）错题 → 单独建衍生
  - 等"高热 + 肌强直"急症错题积累 → 生成 [[完整笔记/专题笔记/USMLE/USMLE_高热肌强直急症鉴别]]（NMS / SS / Malignant catatonia / MH 横向）
  - 等抗精神病药副作用积累 3+ 道 → 生成 [[完整笔记/专题笔记/_衍生_精神科用药副作用对照]]（招牌副作用 → 该查什么映射）

---

## ✅ 学习行动

- [ ] 默写 §2 速查金表（时间 + 表现 + 治疗 + buzzwords）
- [ ] 默写 §3 药物表（治什么 / 加重什么）—— 特别是 benztropine 加重 TD
- [ ] 默写 §5 抗精神病药 TD 风险分层（clozapine / quetiapine 最低；risperidone 较高）
- [ ] 训练 §6 读题 SOP：用药史 + 动作类型 + 时间窗 三角锁定
- [x] ✅ 找 2 道 parkinsonism 题做（已完成 — Park1 错 / Park2 对，见 §4.3 锚点）
- [ ] 默写「**称·程·药**」三轴 + TRAP 四联 + **levodopa 在 DIP 中禁忌**的机制
