...
> [!danger] 分母到底除谁？——看"问什么"，不是看场景 ⭐⭐⭐（ARP 反复错命门）
>
> | 问什么 | 指标 | 除以谁 | 害人例（暴露40 / 未暴露10）| 救人例（对照16 / 治疗10）|
> |---|---|---|---|---|
> | **几倍**（强度）| RR / OR / HR | **参照组**（未暴露/未治疗）| ÷ Ru = ÷ **10** → 4 倍 | ÷ Rc = ÷ **16** |
> | **几成**（归因/降幅）| **ARP** / RRR | **高风险那组自己**（大的那个）| ARP ÷ Re = ÷ **40** → 75% | RRR ÷ Rc = ÷ **16** → 37% |
>
> - **铁律一:问"几倍" → 除参照组**（和谁比就除以谁）。害人除未暴露 Ru（小）、救人除对照 Rc。
> - **铁律二:问"几成 / 百分比" → 除高风险组自己（大的那个）**。害人除**暴露组 Re**、救人除**对照组 Rc**。
> - ⚠️ **害人场景同一道题两种除法**:RR 除未暴露 Ru(10)，**ARP 除暴露组 Re(40)**——这就是你 🔴 反复栽的点。
> - **ARP 直觉**:问"暴露者的病里几成赖暴露" → 全集 = **暴露者的病 = Re** → 减完(Re−Ru)必须再 ÷ **Re**。别停在 ARI(30%)、别错成 1/RR(25%)。
...
1. **分母法则 — 先分"几倍 vs 几成"再定分母**（详见顶部「分母到底除谁」框）⭐
   - **占比类（RRR / ARP，问"几成"）→ 除高风险组自己（大的那个）**：救人 Risk_ctrl、害人 **Risk_exp（Re，暴露组）**。
   - **倍数类（RR / OR / HR，问"几倍"）→ 除参照组**：救人 Risk_ctrl、害人 Risk_unexp（Ru，基线）。
   - ⚠️ 害人场景两类分母不同:ARP 除 **Re（大）**、RR 除 **Ru（小）**——别混。
