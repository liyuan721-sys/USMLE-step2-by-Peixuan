...
> [!info] 🧩 HR 深挖
> HR 的完整体系（来源 Cox 回归 / KM 曲线 / censoring / log-rank / 比例风险假设 / 天花板）见 → [[完整笔记/专题笔记/Biostats/Biostats_生存分析_KM_HR_logrank]]。本节只讲 HR 作为"相对 measure"的广告放大角度。
...
## 4.5 Kalaxin 广告全解剖（Q7686 / Q7687）⭐ — 一张广告四个考点

广告:**Kalaxin（kalgatran etexilate）vs 华法林 warfarin**，非瓣膜性房颤（atrial fibrillation, AFib），QUIKFLOW7 试验。同一张广告埋了 4 类经典考点:

### ① 相对放大（卒中获益）
- 广告大字"**38% reduction**" = HR 0.62 → 降幅 = 1−0.62 = **38%（相对脸）**。
- 绝对脸:2.9% → 1.8% = **ARR 仅 1.1 个百分点**。→ 相对漂亮、绝对很小（老套路）。

### ② 复合掩盖（出血）⭐
广告"**lower total bleeding**"（HR 0.91）——但"总出血"这个**复合**掩盖了**方向相反**的组分:

| 出血类型 | HR (95%CI) | 方向 |
|---|---|---|
| Total bleeding 总出血 | 0.91 (0.86–0.96) | Kalaxin ↓ 显著 |
| Life-threatening | 0.75 (0.61–0.94) | ↓ 显著 |
| **Major bleeding** | **0.96 (0.84–1.10)** | **CI 含 1 → 无差异** |
| ICH 颅内出血 | 0.39 (0.21–0.52) | ↓ 显著（大降）|
| **GI 消化道出血** | **1.38 (1.1–1.5)** | **Kalaxin ↑ 显著（变差！）** |

> [!danger] 复合终点掩盖单组分恶化
> "总出血降低"是真的，但**GI 出血其实升高了（HR 1.38）**——复合把这个坏消息盖住了。**看复合获益必须拆开看组分方向**。

### ③ "两组最相似" = CI 含 1（Q7686）⭐
- 问"哪种出血两组**最相似/无差异**" → 找 **HR≈1 且 CI 含 1.0** 的那个 = **Major bleeding HR 0.96 (0.84–1.10)**（CI 跨 1 = 无显著差异）。
- 反射:**"最相似/无差异" = CI 含 1;"最显著/最不同" = CI 离 1 最远且排除 1**。别只凭 HR 数值"看着接近 1"（Total 0.91 也接近 1，但 CI 排除 1 仍显著）。

### ④ 亚组一致性 + 随机化（Q7687）
- "Efficacy consistent across **all major subgroups**" = 亚组一致性宣称（森林图 forest plot 思维:各亚组 HR 方向一致）。
- 配对题 Item 2/2（Q7687）跳出广告数字，问**随机化是否成功 → 看 baseline 特征均衡**（不是看 outcome）。
...
- 🔁 同主题错题/做对题：
  ...
  - [[mistakes/uworld-mistakes_2026-06#^Q7686]] Kalaxin 广告 — "两组最相似" = CI 含 1（§4.5 ②③）
  - [[mistakes/uworld-mistakes_2026-06#^Q7687]] Kalaxin 广告 Item 2/2 — 随机化成功 = baseline 均衡（§4.5 ④）
