...
> [!danger] 定性比例能比什么 / 不能比什么（Q21644 反射）⭐⭐
> - **Sn/Sp = 列指标**（按疾病状态分母）→ **给"哪个比例更高/更低"的文字描述即可直接比较**（不需要样本量、不受 prevalence 影响）。
>   - 例："Test A 病人中阳性比例更高" = A **更敏感**；"无病者中阴性比例更低" = A **特异性更低**。
> - **Accuracy =(TP+TN)/total、PPV = TP/(TP+FP) = 行指标** → **必须知道样本量 + 实际 Sn/Sp 数值 + 患病率**才能算/比较。
>   - 只给定性比例 → **不能**对 accuracy / PPV 下任何结论。
> - 一句话：**「列指标看文字就能比；行指标没数字（N + 患病率）一律不能比。」**
