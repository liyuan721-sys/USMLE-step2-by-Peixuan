...
| 相对比 / 归因占比（%）| **RRR** = ARR/Rc = **1 − RR**（÷Rc）| **ARP** = (Re−Ru)/Re = **(RR−1)/RR**（÷Re）|
| 需治/暴露人数 | **NNT** = 1/ARR | **NNH** = 1/ARI |

（Re=暴露组、Ru=非暴露、Rt=治疗组、Rc=对照组风险）

> [!tip] ARP 速算 + 别和 RRI / ARI 混（高频 🔴 错点）
> - **ARP（归因危险度百分比）=(RR−1)/RR**——给 RR 直接套:RR2→50%、**RR4→75%**、RR5→80%、RR10→90%（只取决于 RR）。问"暴露者的病里几成赖暴露 / 可预防"用它，**减完除以暴露组 Re**。
> - **别混**:RRI（相对危险度增加）= RR−1（**÷Ru** 未暴露）;ARI（绝对差）= Re−Ru（减完就停，别当 ARP）。
> - 分母到底除谁 + 完整推导 → [[完整笔记/专题笔记/Biostats/Biostats_6指标决策树]]（#考前必看）
...
> [!tip] 效应修饰 → 精准医学（Q12674）⭐
> 整体阳性、但**只有某个亚组显著驱动**（其余亚组 CI 跨 1 无获益）= effect modification（按该亚组变量）→ 结论是 **精准/个体化医学:只给那个阳性亚组干预才有效**。
> - 例:某药整体降哮喘风险显著，但分层后**只有"母亲基线水平最低"那层**显著 → 应**靶向低基线人群**补充，而非全人群。
> - 反钓饵:① 整体显著 ≠ **人人获益**;② 把无效亚组的绝对差当"相对降低"夸大;③ 据某无效亚组否定整体。
