...
# USMLE_患者安全_Cognitive 诊断错误

> [!info] 本笔记定位
> 从 [[完整笔记/专题笔记/USMLE/USMLE_患者安全与质量改进工具]] (原 v1.3.15 大文件) 拆出，专注 **16 大 Cognitive Bias + Diagnostic Error Prevention + Cognitive Interventions + 新增 Cognitive Overload + CDSS + High Complexity 反 pattern**（B 增量待加）。
> 4 子文件之一（另 3：[[完整笔记/专题笔记/USMLE/USMLE_患者安全_QI工具]] / [[完整笔记/专题笔记/USMLE/USMLE_患者安全_Safety文化与沟通]] / [[完整笔记/专题笔记/USMLE/USMLE_患者安全_HFE与药物HAC]]）。
> 主索引：[[完整笔记/专题笔记/USMLE/USMLE_患者安全与质量改进工具]]。

## §0 本文件目录

- §13.4 Cognitive Biases — 16 大 Bias 配对题（含 Anchoring vs Framing 深度区分 v1.3.7/8）
- §13.5 Diagnostic Error Prevention — Cognitive Interventions
- §13.4.x Implicit Bias（与 [[完整笔记/专题笔记/USMLE/USMLE_SDOH_Health_Equity]] 互补）
- **§13.4.x Cognitive Overload = bias enabling factor**（v2.0 — Q106048）
- **§13.4.y Cognitive Bias vs Knowledge Deficiency 边界表** ⭐⭐⭐⭐（v2.0 — Q18796/Q106048/Q106049 反复考点 — "知不知道"是分水岭 / training 反 pattern 警示）
- **§13.4.x High Patient Complexity 反 pattern**（v2.0 — Q106048 反直觉陷阱）
- **§13.5.x CDSS / EDS Tools 3 大机制 + Diagnostic Error 干预匹配表**（v2.0 — Q106049 linked Item 2）
...
| 反 pattern | 例 | 为什么错 |
|---|---|---|
| **Knowledge deficiency** ⭐ 反复考 | Q18796 C 选项 + Q106048 / Q106049 同型 | **Atypical MI in women 已是 well-known**；不是 knowledge 缺；是**反射没建立**（cognitive bias 导致没去想 MI）。完整边界表见 [[#§13.4.y Cognitive Bias vs Knowledge Deficiency — 核心边界表 ⭐⭐⭐⭐（v2.0 增量 — Q18796 + Q106048 反复考点）|§13.4.y 边界表]]（"知不知道"是分水岭 + training 反 pattern）|
...
## §13.4.x Cognitive Overload = Bias Enabling Factor ⭐⭐⭐（v2.0 增量 — Q106048 锚定）

> [!danger] 核心铁律
> **Cognitive bias 不会凭空发生 — Cognitive overload (busy / high-volume / multitasking / time pressure / shift fatigue) 是 bias 激活的 enabling factor。** 当 stem 出现"busy urgent care / high patient volume / multitasking" → root cause = cognitive overload。

### Stem 关键词谱（→ 0.3 秒锁 cognitive overload）
...
### 数据题算占比方法 ⭐（Q106048 stem 模式）

数据题不读"看上去合理"的选项，**算占比** → 哪个 mechanism 解释 majority。
例 Q106048：表格 4 行错误占比：
- ~40% red flags **not documented** → **Availability bias**（常见 dx 太 available → 第一反应想 musculoskeletal）
- ~40% red flags noted but **no follow-up tests** → **Confirmation bias**（强化 presumed dx + 忽略 alternate）
- ~10% no f/u after concerning tests → system / communication
- ~10% patient missed appt → patient-side

**80% (40+40) 都是 cognitive bias → root cause = cognitive overload**。
干扰项 "team communication" 只解释 ~10% → 占比不够。

### Cognitive Bias 4 大与表格行的映射 ⭐

| Bias | 表格表现 | 占比 |
|---|---|---|
| **Availability** | "常见 dx 太 available" → 没记录 red flag | ~40% |
| **Confirmation** | 一旦怀疑某 dx → 不 order 反证 test | ~40% |
| Anchoring | Fixate on initial impression（related to confirmation + premature closure）| — |
| Framing | 外部信息呈现影响判断 | — |

### High Patient Complexity 反 Pattern ⭐⭐（反 USMLE 直觉陷阱 — Q106048 干扰项 B）

> [!warning] 反直觉陷阱
> **"复杂患者 → 漏诊"是错的反向直觉。实际：High patient complexity 反而 PROMOTE comprehensive workup**（医生更 careful，order 更多 dx）→ **不是 cognitive bias 诱因**。
> CK 看到选项 "high patient complexity → diagnostic error" → **闭眼排除**。
...
Cognitive bias 激活（availability + confirmation = 80%）
    ↓ 导致
Diagnostic error (e.g., SEA delayed dx)
```

---

## §13.4.y Cognitive Bias vs Knowledge Deficiency — 核心边界表 ⭐⭐⭐⭐（v2.0 增量 — Q18796 + Q106048 反复考点）

> [!danger] 反复考的核心 trap
> **"医生知不知道"是分水岭。** Knowledge deficiency = 信息**根本不在脑里**；Cognitive bias = 信息**在脑里但没被调用**（思维 shortcut 跳过）。**两者干预完全不同 — 误判 root cause = 选错干预**。

### 13.4.y.1 4 维边界对照表 ⭐⭐⭐

| 维度 | **Knowledge Deficiency（知识缺陷）** | **Cognitive Bias（认知偏倚）** |
|---|---|---|
| 医生是否知道正确答案 | **不知道** — 信息根本不在脑里 | **知道** — 信息在脑里，但没被调用 |
| 问题层级 | **信息层**（input 缺失）| **处理层**（思维 shortcut 跳过）|
| Stem 信号词 | "intern" / "trainee" / "unfamiliar with rare disease" / "first encounter" / "新指南刚出未学" | "**experienced** physician" / "**busy** clinic" / "atypical presentation **in known disease**" / "missed despite **documented** red flags" / "**well-known** condition" |
| 正确干预 | **Education / training / CME / didactic** | **CDSS / cognitive forcing strategies / 减 cognitive overload / 强制 checklist** |
| 反 pattern 干预 | CDSS（信息不在脑，工具也没用）| Training（信息已在脑，再 train 也是 add burden）|

### 13.4.y.2 类比帮记 ⭐

```
Knowledge deficiency = 字典里没有这个词
Cognitive bias       = 字典里有这个词，但你写文章时自动跳过没翻
```

→ Knowledge 缺需要**编词典**（教育）；bias 需要**翻词典提醒**（CDSS / forcing）

### 13.4.y.3 Stem 反射钩（CK 高频）⭐⭐⭐

| Stem 关键词组合 | 锁的 root cause | 锁的干预 |
|---|---|---|
| **Well-known disease** + experienced physician + 漏诊 | **Cognitive bias** | CDSS / cognitive forcing |
| **Atypical presentation of well-known disease** + 漏诊 | **Cognitive bias**（atypical 已 well-described）| CDSS / training "atypical patterns" 也 OK |
| **Rare / newly described disease** + 漏诊 | **Knowledge deficiency** | Education / CME |
| **Busy clinic / multitasking** + 漏诊 well-known disease | **Cognitive overload → bias** | 减负 + CDSS |
| **Trainee / intern** + 漏诊 standard knowledge | **Knowledge deficiency** | Supervision + training |
| **Documented red flags but not acted on** | **Confirmation bias**（信息有，没用）| CDSS prompts |
| **Red flags not even documented** | **Availability bias**（automaticity 跳过）| CDSS reminders + cognitive forcing |

### 13.4.y.4 Q-案例对照（反复错锚定）⭐

| Q# | Stem 类型 | 干扰项 "Knowledge deficiency" 为什么错 |
|---|---|---|
| **Q18796** | Atypical MI in women 漏诊 | **Atypical MI in women 已是 well-known 几十年** — 不是 knowledge 缺；是反射没建立（bias）|
| **Q106048** | SEA red flags 漏诊 in busy urgent care | **SEA red flags 教科书必学** — 不是不知道；是 cognitive overload → bias |
| **Q106049** | linked Item 2：选干预 | Choice D simulation-based **training** = knowledge deficiency 思路 → 错；本题 cause 是 cognitive overload → 选 CDSS |

### 13.4.y.5 反 Pattern 警示 — "Training" 是 Cognitive Bias 题的经典干扰项 ⭐⭐⭐

> [!warning] CK trap 高频公式
> **"Atypical / well-known disease" + "漏诊" + 选项含 "training / simulation / education"** → **直接排除 training**
> - Training 适用 knowledge deficiency
> - Cognitive overload 已是 burden → 加 training = **add more burden 加剧 overload**（见 [[#§13.5.x Diagnostic Error 干预匹配表 + CDSS 3 大机制 ⭐⭐⭐（v2.0 增量 — Q106049 锚定 — Linked vignette Item 2 of Q106048）|§13.5.x CDSS 干预匹配]]）

### 13.4.y.6 与 §13.4.x Cognitive Overload 关系图

```
                    ┌─ Knowledge Deficiency (信息缺) → Training
Root cause of error ─┤
                    └─ Cognitive Bias (信息有未调用)
                          ↑ enabling factor
                       Cognitive Overload (busy / multitasking)
                          ↓ 干预
                       CDSS + 减负 + forcing strategies (不是 training)
```

### 13.4.y.7 Memory Hook

> [!success]
> - **"知不知道是分水岭：不知 = knowledge，知而没调 = bias"** ⭐⭐⭐
> - **"Well-known + experienced + 漏诊 = bias（永不选 knowledge deficiency / training）"** ⭐⭐⭐
> - **"字典类比：knowledge = 没收录这个词；bias = 收录了但没翻"**
> - **"Training 加在 cognitive overload = add burden 反 pattern"**
> - **"Rare disease + 漏诊 / Trainee + 漏诊 standard = 才选 knowledge deficiency"**

---

## §13.5.x Diagnostic Error 干预匹配表 + CDSS 3 大机制 ⭐⭐⭐（v2.0 增量 — Q106049 锚定 — Linked vignette Item 2 of Q106048）

> [!danger] 核心铁律
> **当数据已识别 root cause（Item 1）→ Item 2 选 cause 对应干预；不要再选 RCA "找一次 cause"。Cognitive overload → CDSS / EDS Tools，不是 training（add burden）、不是 RCA（causes 已知 ≠ unclear）。**

### Diagnostic Error 干预匹配表（cause → intervention）⭐⭐⭐
...
- CK trap：simulation-based training 选项在 cognitive overload 题中 = 表面合理实则反向

### Linked Vignette 双题协同模式 ⭐

- **Item 1 = identify root cause**（cognitive overload）
- **Item 2 = 对应干预**（CDSS 减 cognitive overload）
- **正确 logic chain**：cause → 匹配干预；选 RCA = 倒退到 "再找一次 cause"

### CDSS 定义 + 别名（基础术语） ⭐

> [!info] 与 §13.6.1 互补关系
> 本节聚焦 **CDSS 在 Diagnostic Error / Cognitive Overload 干预中的角色**（7 联临床形式 + 反 pattern）。
> CDSS 在 **medication error detect vs 不能 detect 4 类** 的深度框架（Q18754 锚）见 [[完整笔记/专题笔记/USMLE/USMLE_患者安全_HFE与药物HAC#13.6.1 Clinical Decision Support Systems (CDSS) 完整框架 ⭐⭐（v1.3.5 新增 — Q18754 锚定）|HFE 子文件 §13.6.1]]。

- **CDSS** = **Clinical Decision Support System / Software（临床决策支持系统）**
- 别名 **EDS Tools** = **Electronic Decision Support Tools**
- **定义**：集成在 **EMR** 里的 **实时交互式工具** — 在医生做临床决策的瞬间，给出基于证据的**提醒 / 警示 / 推荐**
- 与 **CPOE**（Computerized Provider Order Entry）通常**联用** → CPOE + CDS 是 medication safety 黄金组合

### CDSS 临床形式 7 联表 ⭐⭐⭐（CK 高频考点）

| 形式 | 例子 | 主要解决问题 |
|---|---|---|
| **Drug-drug interaction alert** | 开 warfarin 时 pop-up "patient on TMP-SMX = ↑ INR" | Medication error |
| **Allergy alert** | 开 PCN 时 pop-up "documented PCN allergy" | Anaphylaxis 防 |
| **Order sets / bundles** | "Sepsis order set" 一键 order 1-hour bundle（broad-spectrum abx + lactate + 30mL/kg fluid）| Evidence-based care + 减 omission |
| **Diagnostic algorithms** | Wells score / CURB-65 / FRAX 自动计算 | Diagnostic accuracy |
| **Hard stop**（强制阻断）| Renal dose adjustment 必须填才能 submit order；double-check insulin 强制 second sign-off | Latent error 防 + Tier 3 forcing function |
| **Screening reminders** | 患者 50 岁 → "due for colonoscopy" / "due for AAA screening (men 65-75 ever smoked)" | Preventive care omission |
| **Vaccination prompts** | 65 岁 → "PPSV23 + PCV15/20 due"；arrival → 流感季 → influenza vaccine | Vaccination compliance |

### CDSS 在 HFE 3 档 Reliability 里的位置 ⭐

| Tier | 工具 | 强度 |
|---|---|---|
| Tier 1 | Standardization / education / checklists | 弱 |
| **Tier 2** ⭐ | **CDSS / Memory aids / Decision support**（reminders + alerts + algorithms）| 中 |
| **Tier 3** ⭐ | **Hard stop / Automation / Forcing functions**（CDSS 也可实现 hard stop 进入 Tier 3）| 最强 |

> [!tip] CDSS 跨 Tier 2-3
> 普通 reminder = Tier 2；**hard stop**（必填才能 submit）= 把 CDSS 升级到 Tier 3 forcing function。

### CDSS 反 Pattern — 不适用场景 ⭐⭐⭐

> [!warning] CDSS 不是万能 QI 工具，3 大反 pattern：

| Root cause | 不选 CDSS，应选 | 为什么 |
|---|---|---|
| **Knowledge deficiency** | **Training / CME / didactic** | 信息根本不在脑里 → 工具提醒也没用（见 [[#§13.4.y Cognitive Bias vs Knowledge Deficiency — 核心边界表 ⭐⭐⭐⭐（v2.0 增量 — Q18796 + Q106048 反复考点）|§13.4.y 边界表]]）|
| **Communication gap**（hesitate / hierarchy / fear）| **Closed-loop / SBAR / huddles / psych safety** | CDSS 不解决人际沟通 + emotional barriers |
| **System redesign / process gap**（已知 cause）| **PDSA + FMEA** | CDSS 是工具，不是流程重设计方法论 |

### Alert Fatigue — CDSS 过度使用的反 Pattern ⭐

> [!danger] CDSS over-alerting 会自毁
> - **Alert fatigue**：CDSS 过多 pop-up → 医生 ignore（与 80% 都贴 high-risk wristband 一样 — 失去 discrimination）
> - 解决：**tailored alerts**（high specificity + clinical relevance）> universal alerts
> - 类似反 pattern：universal screening tool low PPV（Q107151 fall screening 80% high-risk）
...
Stem 关键词 → 0.3 秒锁
─────────────────────────────────────────────
"Busy clinic + diagnostic error + 选 CDSS / EDS"     → CDSS 优先
"Sound-alike drug + medication error"                → CPOE + CDSS（drug-drug + allergy alert）
"Cause 已知 = cognitive overload + 选干预"           → CDSS（不选 RCA / 不选 training）
"Renal dose / insulin / chemo + must check"          → CDSS hard stop（Tier 3 forcing function）
...
- 📚 **主笔记 / 跨学科**：
  ...
  - [[完整笔记/专题笔记/USMLE/USMLE_SDOH_Health_Equity]]（patient-side 与 provider-side implicit bias 互补）
