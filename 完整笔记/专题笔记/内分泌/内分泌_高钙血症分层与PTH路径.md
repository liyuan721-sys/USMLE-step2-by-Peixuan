...
> **ALP 特殊**：MM 溶骨是"成骨细胞被 DKK1 抑制"→ **碱性磷酸酶 (ALP) 典型正常**（别因骨破坏就期待 ALP 升）；对比 PTHrP 体液性 / 成骨性转移 ALP 多↑。机制详见 [[完整笔记/专题笔记/内分泌/内分泌_骨质疏松继发病因筛查与骨代谢标志物]] §三。
...
- 🌱 TODO：PTH 非依赖内部"磷"二分 ✅ 已整合（见 §四 warning + §六）；ALP 差异 ✅ 已整合并订正（见 §四 MM danger）——**订正要点：ALP = 成骨细胞活性；MM 溶骨因成骨被 DKK1 抑制 → ALP 反而正常（是溶骨里的例外，不是"溶骨 ALP 多↑"）**；PTHrP 体液性 ALP 常正常、成骨性转移 ALP↑。完整机制 → [[完整笔记/专题笔记/内分泌/内分泌_骨质疏松继发病因筛查与骨代谢标志物]] §三
