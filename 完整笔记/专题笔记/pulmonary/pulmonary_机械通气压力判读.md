...
> [!info] 为什么"非呼气暂停不可"——压力传感器在气道口
> 呼吸机压力传感器装在**气道开口 (airway opening)**，不是肺泡里。呼气还在进行时（气流 ≠ 0），气道口与肺泡之间隔着一段**气道阻力造成的压差**，传感器被它掩盖 → 读到的≈设定 PEEP，**看不到肺泡里困住的 auto-PEEP**。
> **呼气暂停 (expiratory hold) 关阀让气流 = 0 → 阻力压差消失 → 气道口压瞬间等于肺泡压**，这才读得到真实的呼气末肺内压。
> 与吸气暂停同一个物理（§一）：**no-flow 才能让气道口压 = 肺泡压**——吸气端读到平台压，呼气端读到 total PEEP。
>
> 公式：**auto-PEEP = Total PEEP（呼气暂停测得） − Set PEEP（呼吸机设定）**
> - Set PEEP / extrinsic PEEP（外加）= 你设的值
> - auto-PEEP / intrinsic PEEP（内源性）= 气体陷闭多出来的那部分
> - 三者关系：Total = Set + auto
